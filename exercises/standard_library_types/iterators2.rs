...
pub fn capitalize_words(input: Vec<&str>) -> Vec<String> {
    input.iter().map(|&i| capitalize_first(i)).collect()
}

pub fn capitalize_words_string(input: Vec<&str>) -> String {
    capitalize_words(input).join("")
}
...
mod tests {
    ...
    fn test_iterate_into_string() {
        let words = vec!["hello", " ", "world"];
        let capitalized_words: String = capitalize_words_string(words);
        assert_eq!(capitalized_words, "Hello World");
    }
}
